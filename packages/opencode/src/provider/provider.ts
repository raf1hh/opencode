  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        // auth_provider lets a custom provider reuse another provider's auth loader while storing
        // its credentials under its own id, so load the plugin's own provider plus any aliases.
        for (const plugin of plugins) {
          ...
          const load = (id: string) =>
            Effect.gen(function* () {
              ...
              const options = yield* Effect.promise(() =>
                loader(() => bridge.promise(auth.get(providerID).pipe(Effect.orDie)) as any, toPublicInfo(info)),
              )
              const opts = options ?? {}
          ...
          yield* load(source)
          for (const id of aliases(cfg, source)) {
            yield* load(id)
          }
        }
        ...
        // auth_provider aliases reuse their driver's model/vars loaders
        for (const [id] of configProviders) {
          const providerID = ProviderV2.ID.make(id)
          if (disabled.has(providerID)) continue

          const sourceID = ProviderV2.ID.make(driver(cfg, id))
          ...
          if (varsLoader) varsLoaders[providerID] = varsLoader
        }
